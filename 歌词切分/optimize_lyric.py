...
for i in range(0, len(lines)):
    line = lines[i]
    time: str = line.split(']')[0][1:]
    try:
        next_line_time: str = lines[i+1].split(']')[0][1:]
        if '　' in line or ' ' in line:
            split_char = '　' if '　' in line else ' '
            split_char_times = line.count(split_char)
            if split_char_times > 4:
                # 当 代 互 联 网 天·下·第·一！！！
                continue
            # 将一句歌词拆分成两句
            min = time.split(':')[0]
            ...
            new_line_0 = '[' + time + ']' + line.split(']')[1].split(split_char)[0]
            new_line_1 = new_time + split_char.join(line.split(']')[1].split(split_char)[1:])
