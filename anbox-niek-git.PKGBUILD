...
replaces=('anbox-launchers' 'anbox-git')
conflicts=('anbox-launchers' 'anbox-git')
makedepends=('cmake' 'git' 'glm' 'boost' 'properties-cpp' 'gtest' 'python' 'systemd' 'dbus')
source=("git+https://github.com/T2hhbmEK/anbox.git#branch=niek"
...
build() {
  cmake -B build -S "${pkgname%*-niek-git}" \
...
package() {
  ...
  install -Dm 644    "${pkgname%*-niek-git}/snap/gui/icon.png"        "$pkgdir/usr/share/pixmaps/anbox.png"
  install -Dm 644 -t "$pkgdir/usr/share/desktop-directories"     "${pkgname%*-niek-git}/data/desktop/anbox-android.directory"
  install -Dm 644 -t "$pkgdir/etc/xdg/menus/applications-merged" "${pkgname%*-niek-git}/data/desktop/anbox-android.menu"
}
